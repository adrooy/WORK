...
def iplay_ad_info(request):
    ...
    if type != '0':
        infos = IplayAdInfo.objects.filter(channels__contains=channel, type=type).order_by('order_num')
    else:
        infos = IplayAdInfo.objects.filter(channels__contains=channel).order_by('order_num')
    log.debug(infos)
    num = 1
    for info in infos:
        info.order_num = num
        info.type = str(info.type)
        IplayAdInfo.objects.filter(id=info.id).update(order_num=info.order_num)
        num += 1
...
def edit_iplay_ad(request):
    ...
    type = request.GET.get('type')
    if id:
        info = IplayAdInfo.objects.get(id=id)
        chs = info.channels.split(',')
...
def add_iplay_ad(request):
    ...
    #新增渠道或者修改广告信息
    if id:
        IplayAdInfo.objects.filter(id=id).update(type=type, package_name=package_name, title=title, content=content, pic_url=pic_url,
                                                 ...
                                                 filter_pkg_names=filter_pkg_names, channels=chs, life_time=life_time, monet_silent=monet_silent
                                                 , wifi_silent=wifi_silent, download_dismissible=download_dismissible, install_dismissible=install_dismissible)
    #新增广告
    else:
        IplayAdInfo(type=type, package_name=package_name, title=title, content=content, pic_url=pic_url,
                    big_pic_url=big_pic_url, target_type=target_type, target=target, pkg_name=pkg_name
                    , ver_name=ver_name, size=size, filename=filename, show_delay=show_delay, next_delay=next_delay,
                    filter_pkg_names=filter_pkg_names, channels=chs, order_num=0, life_time=life_time, monet_silent=monet_silent
...
def del_iplay_ad(request):
    ...
    IplayAdInfo.objects.filter(id=id).update(channels='')
    response.write('yes')
    return response
...
def search_game(request):
    ...
    if keyword:
        ...
        if not pkgs:
            try:
                pkgs = IplayGamePkgInfo.objects.filter(pkg_name__contains=keyword)
            except:
                pass
        if not pkgs:
            try:
                pkgs = IplayGamePkgInfo.objects.filter(game_name__contains=keyword)
            except:
                pass
        if not pkgs:
            try:
                pkgs = IplayGamePkgInfo.objects.filter(display_name__contains=keyword)
            except:
                pass
...
@csrf_exempt
@login_required
def release_ad_info(request):
    username = request.user
    user = User.objects.get(username=username)
    release = IplayReleaseList.objects.filter(type=2, is_finished=0)
    editor = IplayEditorInfo.objects.get(name=user.last_name+user.first_name)
    infos = IplayReleaseList.objects.filter(type=2).order_by('-id')
    return render_to_response('release_ad.html', {
        'release': release,
        'infos': infos,
        'editor': editor,
    }, context_instance=RequestContext(request))
...
@csrf_exempt
@login_required
def release_ad(request):
    response=HttpResponse()
    response['Content-Type'] = 'text/string'
    IplayReleaseList(start_date=int(time.time()), type=2, worker='52', is_finished=0).save()
    IplayReleaseList(start_date=int(time.time()), type=2, worker='206', is_finished=0).save()
    os.system('python /home/mgmt/operation_shell/operation_release_data/release_ad.py &')
    log.debug('python /home/mgmt/operation_shell/operation_release_data/release_ad.py &')
    response.write('正在发布')
    return response
...
@csrf_exempt
@login_required
def release_game_info(request):
    username = request.user
    user = User.objects.get(username=username)
    release = IplayReleaseList.objects.filter(type=1, is_finished=0)
    editor = IplayEditorInfo.objects.get(name=user.last_name+user.first_name)
    infos = IplayReleaseList.objects.filter(type=1).order_by('-id')
    return render_to_response('release_game.html', {
        'release': release,
        'infos': infos,
        'editor': editor,
    }, context_instance=RequestContext(request))
...
@csrf_exempt
@login_required
def release_game(request):
    """
    新增游戏发布任务：　下发到多个worker 52 206
    :param request:
    :return:
    """
    response=HttpResponse()
    response['Content-Type'] = 'text/string'
    IplayReleaseList(start_date=int(time.time()), type=1, worker='52', is_finished=0).save()
    IplayReleaseList(start_date=int(time.time()), type=1, worker='206', is_finished=0).save()
    os.system('python /home/mgmt/operation_shell/operation_release_data/release_game.py &')
    log.debug('python /home/mgmt/operation_shell/operation_release_data/release_game.py &')
    response.write('正在发布')
    return response
