    ...
    def post(self, fid, subject, content):
        try:
            ...
            if tidurl is None:
                ...
                #p://www.ggzs.me/forum/thread-170101-1-1.html
                #tidstr = re.findall(tid_mid, tidurl)
...
if __name__ == '__main__':
    ...
    cursor.execute("select * from iplay_game_forecast_info where tid = 0")
    n = 0
    for row in cursor.fetchall():
        ...
        if tcnt > 0:
            ...
            print [int_exist_tid]
            print [exist_posturl]
            print "update iplay_game_forecast_info set tid = %d, post_url = '%s' where game_id = '%s'" % (int_exist_tid, exist_posturl, gameId)
            cursor.execute("update iplay_game_forecast_info set tid = %d, post_url = '%s' where game_id = '%s'" % (int_exist_tid, exist_posturl, gameId))
        ...
        if inttid != 0:
            ...
            print [inttid]
            print [posturl]
            cursor.execute("update iplay_game_forecast_info set tid = %d, post_url = '%s' where game_id = '%s'" % (inttid, posturl, gameId))
